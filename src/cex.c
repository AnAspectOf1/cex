...
struct _cex_exc_ctx* _cex_cur_ctx = 0;
struct _cex_exc_ctx* _cex_cur_tcc = 0;
...
void _cex_raise( cex_t code, void* data, cex_free_t free ) {
	CEX_ASSERT( _cex_cur_ctx == 0, "Can not raise outside any try block." );
	...
	CEX->free = free;
	printf( "test: %d\n", 2 );
	longjmp( _cex_cur_ctx->jmp, _cex_mode_raise );
}
...
int _cex_new_ctx() {
	...
	new_ctx->prev = _cex_cur_ctx;
	new_ctx->mode = _cex_mode_begin;

	_cex_cur_ctx = new_ctx;
	_cex_cur_tcc = new_ctx;
...
int _cex_worker() {printf( "jallos %d\n", _cex_cur_tcc->mode );
	/* The beginning */
	if ( _cex_cur_tcc->mode == _cex_mode_begin ) {
		_cex_cur_tcc->mode = _cex_mode_try;
		return 1;
	}

	/* The try block has completed succesfully. */
	else if ( _cex_cur_tcc->mode == _cex_mode_try )
		_cex_cur_ctx = (struct _cex_exc_ctx*)_cex_cur_ctx->prev;

//Fix: The _cex_cur_ctx variable is not pointing to the context to which to read to mode from.
	/* An exception has been raised. */
	else if ( _cex_cur_tcc->mode == _cex_mode_raise ) {
		_cex_cur_ctx = (struct _cex_exc_ctx*)_cex_cur_ctx->prev;
		_cex_cur_tcc->mode = _cex_mode_catch;
		return 1;
	}
